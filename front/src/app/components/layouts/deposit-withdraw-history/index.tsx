...
export const DepositWithdrawHistory = withRootStore(
    observer(
        class DepositWithdrawHistoryClass extends Layout<IProps> {
            protected columns: Array<ColumnProps<ISendTransactionResult>> = [
                {
                    className:
                        'sonm-dw-tx-list__cell-time sonm-dw-tx-list__cell',
                    dataIndex: 'timestamp',
                    title: 'Time',
                    render: (time, record) => {
                ...
                },
                {
                    ...
                    title: 'Operation',
                    className:
                        'sonm-dw-tx-list__cell-action sonm-dw-tx-list__cell',
                    render: (_, record) => {
                        const cls = `sonm-dw-tx-list__cell-action--${
                            record.action
                        }`;
                ...
                },
                {
                    dataIndex: 'fee',
                    className:
                        'sonm-dw-tx-list__cell-fee sonm-dw-tx-list__cell',
                    title: 'Fee',
                    render: (_, record) => {
                        return !record.fee ? (
                            '---'
                ...
                },
                {
                    ...
                    title: 'TxHash',
                    className:
                        'sonm-dw-tx-list__cell-tx-hash sonm-dw-tx-list__cell',
                    render: (_, record) => {
                ...
                },
                {
                    ...
                    title: 'Status',
                    className:
                        'sonm-dw-tx-list__cell-status sonm-dw-tx-list__cell',
                    render: (_, record) => {
            ...
            public render() {
                ...
                return (
                    <div className={cn('sonm-dw-history', className)}>
                        ...
                        >
                            {DepositWithdrawHistoryClass.operations.map(x => (
                                <Option
                                    key={x.value}
                                    value={x.value}
                                    title={x.name}
